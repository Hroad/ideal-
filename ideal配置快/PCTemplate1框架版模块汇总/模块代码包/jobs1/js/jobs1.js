    ...
    coapi.getJobList(params, function (data) {
        ...
        if (data.resultbody.length <= 0) {
            ...
            for (var i = 0; i < data.resultbody.joblist.length; i++) {
                ...
                html += '<li><a jobid="' + data.resultbody.joblist[i].jobid + '" class="job-a">查看详情</a><p class="jobname">' + data.resultbody.joblist[i].jobname + '</p><span> | </span><p class="jobarea">' + data.resultbody.joblist[i].jobareaname + '</p></li>'
            }
            joblist.html(html)
        }
...
$(document).on('click', ".job-a", function (e) {
    ...
    coapi.getJobDetail(jobid, function (data) {
        ...
        var html = "";
        html = '<h3>' + data.resultbody.jobname + '<span>' + data.resultbody.jobareaname + '</span></h3>' + data.resultbody.jobinfo + '<a class="apply" jobid="' + data.resultbody.jobid + '">投递简历</a><a class="back">返回列表</a><span class="clear"></span>'
        $('.jobmenu').html(html)
    });
    $('.joblist').hide()
    $('.jobmenu').fadeIn(300)
