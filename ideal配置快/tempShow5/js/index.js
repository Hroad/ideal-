...
function swichImg(_index, index) {
    ...
    switch (_index) {
        ...
        case 2:
            // jobs页面的特殊情况
            // if (index == 1) {
            //     $(".show_box img").attr("src", "images/jobs" + index + "_1.png");
            //     $(".show_box").append("<img class='nowImg' src='images/jobs" + index + "_2.png' />");
            // }
            // if (index == 2) {
...
$(".select").click(function () {
    $(this).children(".opt_box").slideToggle();
    $(this).siblings(".select").children(".opt_box").slideUp();
})
...
$(".opt").click(function (e) {
    ...
    if ($(this).index() <= 1) {
        ...
        $(this).parent(".opt_box").slideUp();

        $(this).addClass("active").siblings().removeClass("active");

        var index = $(this).parent().parent().index();

        var thisNav = $(".nav li").eq(index - 1);

        thisNav.addClass("active_bg").siblings().removeClass("active_bg");
        ...
        // 小图标替换
        thisNav.children(".left_icon").children(".icon").attr("src", "images/icon" + (index * 1) + "_on.png");

        $(".show_box").show();

        $(".stylesheet_box").show();

        swichImg(index - 1, $(this).index() + 1);

        $(".style1").eq($(this).index()).addClass("active").siblings().removeClass("active");

        // 父盒子内容替换
        $(this).parent(".opt_box").siblings("span").text($(this).text());
